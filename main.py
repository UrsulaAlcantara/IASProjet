...
def bootstrapped_error(func, tab, it):
    """
    Compute bootstrapped error of func(tab) with it iteration
    """
    v_tab = []
    for _ in range(it):
...
def main():
    ...
    # correct sign prediction test
    Y_test_sup0 = Y_test > 0
    Y_test_pred_sup0 = Y_test_pred > 0

    correct_pred = Y_test_sup0 == Y_test_pred_sup0
    print(
        f"Taux de bonnes predictions pos/neg: {correct_pred.mean()}, bootstrapped err: {bootstrapped_error(np.mean, correct_pred, 10000)}")
